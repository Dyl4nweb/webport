  ...
  return (
    ...
    >
      <head>
          ...
          dangerouslySetInnerHTML={{
            __html: `
              ...
              @media (hover: hover) and (pointer: fine) {
                html:not([data-admin]), html:not([data-admin]) * {
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `(function(){try{var doc=document.documentElement;var matchCookie=document.cookie.match(/(?:^|;\\s*)site_active_theme=([^;]+)/);var cookieTheme=matchCookie?matchCookie[1]:null;var localTheme=null;try{localTheme=localStorage.getItem("site_active_theme");}catch(e){}var domTheme=doc.getAttribute("data-theme");var st=(localTheme==="modern"||localTheme==="cafe"||localTheme==="cyber")?localTheme:((cookieTheme==="modern"||cookieTheme==="cafe"||cookieTheme==="cyber")?cookieTheme:((domTheme==="modern"||domTheme==="cafe"||domTheme==="cyber")?domTheme:"cafe"));doc.setAttribute("data-theme",st);try{localStorage.setItem("site_active_theme",st);}catch(e){}try{document.cookie="site_active_theme="+st+";path=/;max-age=31536000;SameSite=Lax";}catch(e){}var storedMode=null;try{storedMode=localStorage.getItem("theme");}catch(e){}var isDark=storedMode==="dark"||(!storedMode&&window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches);doc.classList.remove("theme-swap");var bg=st==="cafe"?(isDark?"#14100c":"#f8f4ed"):(st==="cyber"?(isDark?"#030705":"#e8f4ed"):(isDark?"#000000":"#fbfbfd"));if(isDark){doc.classList.add("dark");doc.classList.remove("light");doc.style.colorScheme="dark";}else{doc.classList.remove("dark");doc.classList.add("light");doc.style.colorScheme="light";}doc.style.backgroundColor=bg;doc.style.setProperty("--splash-bg",bg);}catch(e){}})();`,
          }}
        />
      </head>
